...
def calculate_attack(det_boxes, det_labels, det_scores, true_boxes, true_labels, true_difficulties):
    """
    Calculate the Mean Average Precision (mAP) of detected objects.
    ...
    for c in range(1, n_classes):
        ...
        for d in range(n_class_detections):
            this_detection_box = det_class_boxes[d].unsqueeze(0)  # (1, 4)
            this_image = det_class_images[d]  # (), scalar
            this_score = det_class_scores[d]  # (), scalar
            # Find objects in the same image with this class, their difficulties, and whether they have been detected before
            object_boxes = true_class_boxes[true_class_images == this_image]  # (n_class_objects_in_img)
            object_difficulties = true_class_difficulties[true_class_images == this_image]  # (n_class_objects_in_img)
            # If no such object in this image, then the detection is a false positive
    ...
    # Calculate Mean Average Precision (mAP)
    #mean_average_precision = average_precisions.mean().item()
    n_classes = sum(average_precisions > 0)
    mean_average_precision = average_precisions.sum() / n_classes
